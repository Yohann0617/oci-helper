...
public class BootVolumeServiceImpl implements IBootVolumeService {
    ...
    public Page<BootVolumeListPage.BootVolumeInfo> bootVolumeListPage(BootVolumePageParams params) {
        ...
        try (OracleInstanceFetcher fetcher = new OracleInstanceFetcher(sysUserDTO)) {
            List<BootVolume> bootVolumeList = fetcher.listBootVolume();
            List<BootVolumeListPage.BootVolumeInfo> resList = CollectionUtil.isEmpty(bootVolumeList) ? Collections.emptyList() :
                    bootVolumeList.parallelStream().filter(x -> CommonUtils.contains(x.getDisplayName(), params.getKeyword(), true) ||
