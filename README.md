...
2. 如不使用 docker 部署则再下载一个`oci-helper-1.2.2.jar`文件到`/app/oci-helper`目录下，直接`nohup java -jar oci-helper-1.2.2.jar > /var/log/oci-helper.log &`运行即可（前提是环境上要有`jre8`或`jdk8`环境）。
